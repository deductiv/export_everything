...
require([
	...
	//var uri = Splunk.util.make_url(`/splunkd/__raw/servicesNS/nobody/${app}/deductiv/event_push_setup?output_mode=json`);
	var uri = Splunk.util.make_url(`/splunkd/__raw/servicesNS/nobody/${app}/event_push/event_push_setup?output_mode=json`);
	var config = {};
	...
	$(document).on("click", "#setup_save", function(event){
		...
		Object.keys(fields).forEach(function (section) {
			...
			if (Object.keys(new_config[section]).length > 0 ) {
				// Submit HEC settings
				var uri = Splunk.util.make_url(`/splunkd/__raw/servicesNS/nobody/${app}/event_push/event_push_setup/${section}`);
